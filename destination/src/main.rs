...
// lazy_static::lazy_static! { static ref DATA : DataStore = Arc :: new (RwLock :: new (Data :: default ())) ; }
...
//             let result = guard.repeat(input).await;
...
//             let result = guard.add(input).await;
//             let output = bincode::serialize(&result).unwrap();
//             output
//         })
//     },
...
type DataStore = Arc<RwLock<Data>>;
lazy_static::lazy_static! { static ref DATA : DataStore = Arc :: new (RwLock :: new (Data :: new ())) ; }
type AsyncFn = fn(Vec<u8>) -> Pin<Box<dyn std::future::Future<Output = Vec<u8>> + Send>>;
const N: u32 = 2u32;
static FUNCTIONS: [AsyncFn; N as usize] = [
    |bytes: Vec<u8>| {
